...
public:
    ...
    static IndexedTriangleList<V> GetPlain(int divisions = 7, float size = 1.0f) {
        ...
        std::vector<V> vertices(Math::Square(nVerticesSide + 1));
        {
        ...
        indices.reserve(Math::Square(divisions) * 6);
        {
            const auto vxy2i = [nVerticesSide](size_t x, size_t y) { return y * nVerticesSide + x; };

            for (size_t y = 0; y < divisions; y++) {
                for (size_t x = 0; x < divisions; x++) {
                    const std::array<size_t, 4> indexArray = {vxy2i(x, y),
                        vxy2i(x + 1, y),
                        vxy2i(x, y + 1),
                        vxy2i(x + 1, y + 1)};
